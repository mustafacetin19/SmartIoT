...
import "./DeviceSelectionPage.css"; // ortak stil
    ...
    <div className="page">
      ...
      <form className="card" onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        ...
        <div className="actions">
          <button className="btn primary" type="submit" disabled={saving}>{saving ? "Oluşturuluyor…" : "Oluştur"}</button>
          <button className="btn ghost" type="button" onClick={() => navigate("/select-device")}>Vazgeç</button>
        </div>
      </form>
    </div>
  );
